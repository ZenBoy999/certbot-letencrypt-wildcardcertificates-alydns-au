...
使用 certbot 工具，为不能自动给 letencrypt 通配符证书自动续期（renew）
...
在 certbot 官方提供的插件和 hook 例子中，都没有针对国内 DNS 服务器的样例，所以我（ywdblog)编写了这样一个工具，目前支持阿里云 DNS、腾讯云 DNS、GoDaddy（certbot 官方没有对应的插件）。 
...
$ git clone https://github.com/ZenBoy999/certbot-letencrypt-wildcardcertificates-alydns-au
...
（1）配置 dns.cfg 文件，DNS API 密钥：

这个 API 密钥什么意思呢？由于需要通过 API 操作阿里云 DNS 或腾讯云 DNS 的记录，所以需要去域名服务商哪儿获取 API 密钥，然后配置在 dns.cfg 文件中:

- 阿里云 [API key 和 Secrec 官方申请文档](https://help.aliyun.com/knowledge_detail/38738.html)。
- 腾讯云 [API 密钥官方申请文档](https://console.cloud.tencent.com/cam/capi)。
- GoDaddy [API 密钥官方申请文档](https://developer.godaddy.com/getstarted)。
...
- 第二个参数代表你的DNS厂商(aly|txy|godaddy)
...
![公众号：甬洁网络](https://app.yj777.cn/upload/mini_qrcode/yj_service_qrcode.jpg)
